 ...
 * Flags.Flag.USER ：底层系统是否支持用户自定义标记，应用程序只能检索这个属性，而不能设置这个属性。
 * <p>
 * <p>
 * 获取邮箱邮件夹：
 * -- 获取邮箱下所有文件夹
 ...
 * -- 发件箱
 * IMAPFolder sendFolder = (IMAPFolder) store.getFolder("已发送");
 * -- 草稿箱
 * IMAPFolder folder = (IMAPFolder) store.getFolder("草稿");
 * -- 垃圾邮件箱
 * IMAPFolder sendFolder = (IMAPFolder) store.getFolder("垃圾邮件");
 * -- 已删除邮件
 * IMAPFolder sendFolder = (IMAPFolder) store.getFolder("已删除邮件");
 */
public class MailParsingTool {
     ...
     * 获取收件箱邮件价
     */
    public static Folder getNotReadMsgs(Store store) throws MessagingException {
     ...
     * 将邮件夹全部邮件设为未读
     */
    public static void setMessagesToRECENT(Folder folder, Message[] messages) throws MessagingException {
        folder.setFlags(messages, new Flags(Flags.Flag.RECENT), true);
    }
    ...
    /**
     * 拷贝到新邮件文件，然后删除该邮件，实现拷贝
     */
    public static void moveMessage(IMAPFolder sourceFolder, IMAPFolder targetFolder,MimeMessage messages) throws MessagingException, IOException {
        if (messages == null) {
            return;
        }
        try {
